 ...
 *   Multiple results may be emitted (one per IPC follow-up).
...
import type { ContainerInput } from './copilot-runner.js';

// Re-export so the type is visible to callers that import index.js
...
async function main(): Promise<void> {
  ...
  // Remove any stale _close sentinel from a previous run
  try {
    fs.unlinkSync(IPC_INPUT_CLOSE_SENTINEL);
  } catch {
    /* ignore */
  ...
  // Build the initial prompt, draining any pending IPC messages
