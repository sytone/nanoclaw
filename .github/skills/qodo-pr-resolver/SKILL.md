...
After displaying the table, ask the user how they want to proceed using ask_user:
...
- For each issue marked as "Fix" (starting with CRITICAL):
  ...
  - **Present the fix and ask for approval in a SINGLE step:**
    ...
    5. Immediately use ask_user with these options:
  ...
  - **WAIT for user's choice via ask_user**
  ...
  - **If "Defer" selected:**
    - Ask for deferral reason using ask_user
...
**Single-step approval with ask_user:**
...
**CRITICAL:** Single validation only - do NOT show the diff separately and then ask. Combine the diff display and the question into ONE message. The user should see: brief context → current code → proposed diff → ask_user, all at once.

**Example:** Show location, Qodo's guidance, current code, proposed diff, then ask_user with options (✅ Apply fix / ⏭️ Defer / 🔧 Modify). Wait for user choice, apply via Edit tool if approved.
