...
Use `ask_user` to collect configuration. **Adapt auth options based on environment:**

If IS_HEADLESS=true AND not WSL → ask_user: How do you want to authenticate WhatsApp?
- **Pairing code** (Recommended) - Enter a numeric code on your phone (no camera needed, requires phone number)
- **QR code in terminal** - Displays QR code in the terminal (can be too small on some displays)

Otherwise (macOS, desktop Linux, or WSL) → ask_user: How do you want to authenticate WhatsApp?
...
ask_user: What is your phone number? (Include country code without +, e.g., 1234567890)
...
ask_user: Is this a shared phone number (personal WhatsApp) or a dedicated number (separate device)?
- **Shared number** - Your personal WhatsApp number (recommended: use self-chat or a solo group)
- **Dedicated number** - A separate phone/SIM for the assistant

ask_user: What trigger word should activate the assistant?
- **@Andy** - Default trigger
- **@Claw** - Short and easy
- **@GitHub Copilot** - Match the AI name

ask_user: What should the assistant call itself?
- **Andy** - Default name
- **Claw** - Short and easy
- **GitHub Copilot** - Match the AI name

ask_user: Where do you want to chat with the assistant?
...
The output shows `JID|GroupName` pairs. Present candidates as ask_user (names only, not JIDs).
