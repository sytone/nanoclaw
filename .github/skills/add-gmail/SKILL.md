...
ask_user: Should incoming emails be able to trigger the agent?
...
Once verified, offer filter customization via `ask_user` — by default, only emails in the Primary inbox trigger the agent (Promotions, Social, Updates, and Forums are excluded). The user can keep this default or narrow further by sender, label, or keywords. No code changes needed for filters.
