...
Use `ask_user: Do you have a Parallel AI API key, or should I help you get one?`
...
Find the line:
```typescript
const allowedVars = ['GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)', 'GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)'];
```

Replace with:
```typescript
const allowedVars = ['GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)', 'GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)', 'PARALLEL_API_KEY'];
```
...
**Permission:** ALWAYS use `ask_user` before using this tool

**How to ask permission:**
```
ask_user: I can do deep research on [topic] using Parallel's Task API. This will take 2-5 minutes and provide comprehensive analysis with citations. Should I proceed?
```
