...
src/container-runner.ts               container/agent-runner/
    │                                      │
    │ spawns container                      │ runs GitHub Copilot Agent SDK
...
| **GitHub Copilot sessions** | `~/.claude/projects/` | GitHub Copilot Code session history |
...
### 1. "GitHub Copilot Code process exited with code 1"
...
# GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)=sk-ant-oat01-...  (subscription)
# GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)=sk-ant-api03-...        (pay-per-use)
```
...
**Workaround:** The system extracts only authentication variables (`GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)`, `GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)`) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed.
...
echo '{}' | docker run -i \
  ...
  -c 'export $(cat /workspace/env-dir/env | xargs); echo "OAuth: ${#GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)} chars, API: ${#GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)} chars"'
```
...
```
/workspace/
├── env-dir/env           # Environment file (GitHub Copilot host authentication (~/.config/github-copilot/hosts.json) or GitHub Copilot host authentication (~/.config/github-copilot/hosts.json))
...
### 5. Session Not Resuming / "GitHub Copilot Code process exited with code 1"

If sessions aren't being resumed (new session ID every time), or GitHub Copilot Code exits with code 1 when resuming:
...
grep -A3 "GitHub Copilot sessions" src/container-runner.ts
```
...
### Test GitHub Copilot Code directly:
```bash
docker run --rm --entrypoint /bin/bash \
  -v $(pwd)/data/env:/workspace/env-dir:ro \
  nanoclaw-agent:latest -c '
...
The agent-runner uses these GitHub Copilot Agent SDK options:
...
**Important:** `allowDangerouslySkipPermissions: true` is required when using `permissionMode: 'bypassPermissions'`. Without it, GitHub Copilot Code exits with code 1.
...
docker run --rm --entrypoint /bin/bash nanoclaw-agent:latest -c '
  ...
  echo "=== GitHub Copilot Code version ==="
  claude --version
...
GitHub Copilot sessions are stored per-group in `data/sessions/{group}/.claude/` for security isolation. Each group has its own session directory, preventing cross-group access to conversation history.
...
[ -f .env ] && (grep -q "GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)=sk-" .env || grep -q "GitHub Copilot host authentication (~/.config/github-copilot/hosts.json)=sk-" .env) && echo "OK" || echo "MISSING - add GitHub Copilot host authentication (~/.config/github-copilot/hosts.json) or GitHub Copilot host authentication (~/.config/github-copilot/hosts.json) to .env"
